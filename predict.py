...
def make_predictions(df_road, df_test, arc_name, datetimes):
    ...
    for metric in ["Débit horaire", "Taux d'occupation"]:
        ...
        X_train["Durée avant les prochaines vacances scolaires"] = pd.to_timedelta(
            X_train["Durée avant les prochaines vacances scolaires"]
        ).dt.days
        X_test["Durée avant les prochaines vacances scolaires"] = pd.to_timedelta(
            X_test["Durée avant les prochaines vacances scolaires"]
        ).dt.days
    ...
    return pd.DataFrame(
        {
            "arc": [arc_name] * len(datetimes),
            "datetime": datetimes,
            "debit_horaire": all_preds[0],
            "taux_occupation": all_preds[1],
        }
    )
...
def main():
    ...
    df_ce["Date et heure de comptage"] = pd.to_datetime(
        df_ce["Date et heure de comptage"]
    )
    df_sts["Date et heure de comptage"] = pd.to_datetime(
        df_sts["Date et heure de comptage"]
    )
    df_conv["Date et heure de comptage"] = pd.to_datetime(
        df_conv["Date et heure de comptage"]
    )
    ...
    df_weather_forecast["Date"] = pd.to_datetime(
        df_weather_forecast["date_time"]
    ).dt.date
    ...
    df = df[
        (df["Date et heure de comptage"] < datetime(2022, 12, 14))
        & (df["Date et heure de comptage"] >= datetime(2022, 12, 9))
    ]
