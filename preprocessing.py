"""Fonctions de preprocessing specifiques aux fichiers."""
...
def read_csv_files(data_dir, filenames):
    dfs = [
        pd.read_csv(os.path.join(data_dir, f), sep=";", index_col=0).assign(filename=f)
        for f in filenames
    ]
...
def filter_nodes(df):
    noeuds_amont_aval = {
        ...
        "convention.csv": {"amont": "Lecourbe-Convention", "aval": "Convention-Blomet"},
        "saints-peres.csv": {
...
def convert_dates_to_datetime(df, date_col):
    ...
    df[date_col] = df[date_col].apply(
        lambda d: d.tz_localize(None) + pd.DateOffset(hours=1)
    )
    return df
...
def add_holidays(df, data_dir):
    ...
    df["Jour férié"] = df["Date"].isin(
        pd.to_datetime(df_bank_holiday["date"]).apply(lambda d: d.date())
    )

    df_holiday = pd.read_csv(
        os.path.join(data_dir, "fr-en-calendrier-scolaire.csv"), sep=";"
    )
    df_holiday = df_holiday[df_holiday["location"] == "Paris"]
    df_holiday = df_holiday[
        df_holiday["annee_scolaire"].isin(["2021-2022", "2022-2023"])
    ]
    ...
    df["Vacances scolaires"] = df["Date"].apply(
        lambda d: is_during_holiday(d, df_holiday)
    )
    df["Prochaines vacances scolaires"] = pd.to_datetime(
        df["Date"].apply(lambda d: first_holiday_after(d, df_holiday))
    )
    df["Durée avant les prochaines vacances scolaires"] = (
        df["Prochaines vacances scolaires"] - df["Date et heure de comptage"]
    )
...
def add_weather_data(df, data_dir):
    ...
    return df.merge(
        df_weather,
        left_on="Date et heure de comptage",
        right_on="date_time",
        how="left",
...
def get_weather_forecast(data_dir):
    df_weather_forecast = pd.read_csv(
        os.path.join(data_dir, "weather_forecast_paris.csv")
    )
    df_weather_forecast["date"] = pd.to_datetime(df_weather_forecast["date"])
    df_weather_forecast["date_time"] = df_weather_forecast.apply(
        lambda x: x["date"] + pd.DateOffset(hours=x["time"] / 100), axis=1
    )

    df_weather_forecast = df_weather_forecast[
        ["date_time", "humidity", "tempC", "visibilityKm", "cloudcover", "precipMM"]
    ]
...
def add_weekday_ohe(df):
    df["Jour de la semaine"] = pd.to_datetime(
        df["Date et heure de comptage"]
    ).dt.dayofweek
    df_ohe = pd.concat(
        [df, pd.get_dummies(df["Jour de la semaine"], prefix="Jour de la semaine")],
        axis=1,
